...
{      
	public static async Task Test()
	{
		...
		await using (var writeStream = new StreamWriter(connection.StoreApi.CreateFile(storeId)))
		{
		...
		var file = (await connection.StoreApi.ListFiles(storeId, PagingQueryExtension.DefaultQuery)).ReadItems.First();
		Console.WriteLine($"Created file id: {file.Info.FileId}");
		try
		{
			using var readStream = new StreamReader(await connection.StoreApi.OpenFileForRead(file.Info.FileId));
